...
export default function ProductsManagementPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.name || !formData.hsn_code) {
  ...
  const handleEdit = (p) => {
    setFormData({ name: p.name || "", hsn_code: p.hsn_code || p.hsn || "" });
    setEditId(p.id);
    setShowForm(true);
  };
  ...
  return (
    <div style={{ fontFamily: "Segoe UI, Arial, sans-serif", padding: "8px" }}>
      {showForm ? (
        <div style={card}>
          <div style={headerRow}>
            <h2 style={titleStyle}>{editId ? "Edit Product" : "Add Product"}</h2>
            <button type="button" onClick={resetForm} style={btnPrimary}>Back To Product List</button>
          </div>
          <form onSubmit={handleSubmit}>
            <div style={formGrid}>
              <Field label="Product Name">
                <input name="name" value={formData.name} onChange={handleChange} placeholder="Product Name *" style={inp} />
              </Field>
              <Field label="HSN Code">
                <input name="hsn_code" value={formData.hsn_code} onChange={handleChange} placeholder="HSN Code *" style={inp} />
              </Field>
            </div>
            <div style={actionRow}>
        ...
        <>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "14px", gap: 10, flexWrap: "wrap" }}>
            <h2 style={titleStyle}>Products Management</h2>
            <button type="button" onClick={() => setShowForm(true)} style={{ ...btnPrimary, background: "#0f766e" }}>Add Product</button>
          </div>
          <div style={tableCard}>
              ...
              <tbody>
                {products.map((p, i) => (
                  <tr key={p.id} style={{ background: i % 2 ? "#f8fafc" : "#fff" }}>
                    ...
                    <td style={td}>{p.hsn_code || p.hsn || "-"}</td>
                    <td style={td}>
                      <button type="button" onClick={() => handleEdit(p)} style={{ ...mini, background: "#2563eb" }}>Edit</button>{" "}
                      <button type="button" onClick={() => handleDelete(p.id)} style={{ ...mini, background: "#dc2626" }}>Delete</button>
                    </td>
                  </tr>
                ))}
                {products.length === 0 ? (
                  <tr><td colSpan={4} style={{ ...td, textAlign: "center", padding: "20px" }}>No products found.</td></tr>
                ) : null}
              </tbody>
            </table>
          </div>
