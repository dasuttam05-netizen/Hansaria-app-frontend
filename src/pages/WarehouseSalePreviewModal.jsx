        ...
        {(() => {
          ...
          const handleTransportSave = async () => {
            ...
            try {
              ...
              await API.put(`/api/wh-vouchers/sale/${saleId}`, payload);
              const updated = await API.get(`/api/wh-vouchers/sale/${saleId}/summary`);
          ...
          return (
            <>
              ...
              <div style={{ marginTop: 14, border: "1px solid #d1d5db", borderRadius: 10, overflow: "hidden", background: "#fff" }}>
                <div style={{ padding: "10px 12px", background: "#eef4ff", borderBottom: "1px solid #d1d5db", fontWeight: 800, color: "#111827" }}>Tagged Purchase Details (F10)</div>
                {purchaseLinks.length > 0 ? (
                  <div style={{ overflowX: "auto" }}>
                    <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                      <thead><tr><th style={th}>Purchase Bill</th><th style={th}>Farmer</th><th style={th}>Consignee</th><th style={th}>Qty</th><th style={th}>Rate</th><th style={th}>Purchase Amount</th><th style={th}>Purchase Deduction</th><th style={th}>Net Purchase</th></tr></thead>
                      <tbody>
                        {purchaseLinks.map((item, index) => {
                          const qty = toNumber(item.quantity || item.weight);
                          const rate = toNumber(item.rate);
                          const gross = toNumber(item.amount || qty * rate);
                          const deduction = toNumber(item.total_deduction) || (toNumber(item.claim_amount) + toNumber(item.other_deduction) + toNumber(item.transport_charge) + toNumber(item.cd_amount) + toNumber(item.adjustment_amount) + toNumber(item.tds_amount));
                          const netPurchase = Math.max(gross - deduction + toNumber(item.round_off), 0);
                          return <tr key={`${item.purchase_id || item.voucher_no}-${index}`}>
                            <td style={td}>{item.voucher_no || "-"}</td><td style={td}>{item.farmer_name || preview.farmer || "-"}</td><td style={td}>{item.consignee_name || preview.consignee || "-"}</td><td style={td}>{formatDecimal4(qty)}</td><td style={td}>{formatMoney(rate)}</td><td style={td}>{formatMoney(gross)}</td><td style={td}>{formatMoney(deduction)}</td><td style={{ ...td, fontWeight: 800 }}>{formatMoney(netPurchase)}</td>
                          </tr>;
                        })}
                      </tbody>
                    </table>
                  ...
                  <div style={{ padding: 12, color: "#64748b" }}>No purchase tag found on this sale voucher.</div>
                )}
              </div>

              <div style={{ marginTop: 14, border: "1px solid #d1d5db", borderRadius: 10, overflow: "hidden", background: "#fff" }}>
                <div style={{ padding: "10px 12px", background: "#f3f4f6", borderBottom: "1px solid #d1d5db", fontWeight: 800, color: "#111827" }}>Sale Deduction & Profit</div>
                <div style={{ overflowX: "auto" }}><table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}><tbody><tr><td style={td}>Shortage / Claim</td><td style={td}>{formatMoney(toNumber(preview.claimAmount) || toNumber(preview.shortageAmount))}</td><td style={td}>Freight</td><td style={td}>{formatMoney(transportCharge)}</td></tr><tr><td style={td}>Other Deduction</td><td style={td}>{formatMoney(preview.otherDeduction)}</td><td style={td}>Total Deduction</td><td style={{ ...td, fontWeight: 800 }}>{formatMoney(adjustedTotalDeduction)}</td></tr><tr><td style={td}>Net Sale Value</td><td style={{ ...td, fontWeight: 800 }}>{formatMoney(netPayable)}</td><td style={td}>Profit / Loss</td><td style={{ ...td, fontWeight: 900, color: profitLoss >= 0 ? "#047857" : "#b91c1c" }}>{formatMoney(profitLoss)}</td></tr></tbody></table></div>
              </div>
