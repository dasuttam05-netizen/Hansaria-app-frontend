...
const EmployeeCashBookPage = () => {
  ...
  const filteredRows = useMemo(() => {
    const selectedFundSource = String(appliedFilters.fund_source || "employee_cash");
    return entries
      .filter((e) => e.employee_id)
      .filter((e) => selectedFundSource === "all" ? true : String(e.fund_source || "") === selectedFundSource)
  ...
  }, [entries, employees, appliedFilters]);
  ...
  const handleExportExcel = () => {
    exportLedgerExcel({
      title: "Employee Cash Book Ledger",
      fileName: `employee-cash-book-ledger-${showDeleted ? "deleted" : "active"}.xls`,
      ledgerTitle,
  ...
  const handleExportPDF = () => {
    exportLedgerPDF({
      title: "Employee Cash Book Ledger",
      fileName: `employee-cash-book-ledger-${showDeleted ? "deleted" : "active"}.pdf`,
      ledgerTitle,
      dateFrom: appliedFilters.start_date,
      dateTo: appliedFilters.end_date,
      headers: exportHeaders,
                ...
                {ledgerRows.length === 0 ? (
                  ...
                  ledgerRows.map((entry) => (
                    <tr key={entry.id}>
                      ...
                      <td style={td}>{entry.entry_type ? `${entry.entry_type}` : "-"}</td>
...
export default EmployeeCashBookPage;
