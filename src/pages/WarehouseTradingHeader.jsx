...
export default function WarehouseTradingPage() {
  ...
  const buildPaymentVoucherPdf = (row) => {
    ...
    const header = () => {
      ...
      doc.setFillColor(230, 178, 46);
      doc.rect(0, 32, pageWidth, 2, "F");
      doc.setTextColor(255, 255, 255);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(18);
      doc.text("PAYMENT VOUCHER", margin, 15);
      doc.setFontSize(8.5);
      doc.setFont("helvetica", "normal");
      doc.text("WAREHOUSE TRADING • PAYMENT AGAINST PURCHASE BILL", margin, 23);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(10);
      doc.text(`Voucher No: ${row.voucher_no || "-"}`, pageWidth - margin, 14, { align: "right" });
      doc.setFont("helvetica", "normal");
      doc.text(`Payment Date: ${paymentDate}`, pageWidth - margin, 22, { align: "right" });
    };
    ...
    const ensureSpace = (required = 35, y = 40) => {
    ...
    header();
    let y = 43;
    y = sectionTitle("PARTY & PAYMENT DETAILS", y);
    autoTable(doc, {
      startY: y,
      margin: { left: margin, right: margin },
      theme: "grid",
      styles: { font: "helvetica", fontSize: 8.5, cellPadding: 2.7, lineColor: [210, 222, 230], textColor: [25, 45, 58] },
      columnStyles: { 0: { cellWidth: 43, fontStyle: "bold", fillColor: [238, 247, 248] }, 1: { cellWidth: contentWidth - 43 } },
      body: [
    ...
    // Resolve purchase details from adjustment rows. The backend also supplies
    // purchase_details for older Against payments by matching the reference.
    let purchaseDetails = adjustments
    ...
    if (!purchaseDetails.length && row.purchase_details) {
      const details = Array.isArray(row.purchase_details) ? row.purchase_details : [row.purchase_details];
      purchaseDetails = details.map((purchase) => ({ ...purchase, adjusted_amount: purchase.adjusted_amount ?? adjusted, balance_after_adjustment: purchase.balance_after_adjustment ?? onAccount }));
    }

    if (!purchaseDetails.length && row.purchase_bill_details) {
      const details = Array.isArray(row.purchase_bill_details) ? row.purchase_bill_details : [row.purchase_bill_details];
      purchaseDetails = details.map((purchase) => ({ ...purchase, adjusted_amount: purchase.adjusted_amount ?? adjusted, balance_after_adjustment: purchase.balance_after_adjustment ?? onAccount }));
    }
    ...
    if (purchaseDetails.length) {
      purchaseDetails.forEach((purchase, index) => {
        y = ensureSpace(55, y);
        y = sectionTitle(`PURCHASE DETAILS ${purchaseDetails.length > 1 ? `• BILL ${index + 1}` : ""}`, y);
        ...
        const lorryNo = purchase.lorry_no || purchase.lorry_number || purchase.vehicle_no || "-";
        const purchaseDate = formatLedgerDate(purchase.date || purchase.bill_date || purchase.purchase_date);
        const deductions = [
        ...
        const deductionTotal = toNumber(purchase.total_deduction_calculated ?? purchase.total_deduction ?? deductions.reduce((sum, [, v]) => sum + v, 0));
        const paid = toNumber(purchase.adjusted_amount);
        ...
        autoTable(doc, {
          ...
          margin: { left: margin, right: margin },
          theme: "grid",
          styles: { font: "helvetica", fontSize: 8.3, cellPadding: 2.6, lineColor: [205, 220, 226], textColor: [25, 45, 58] },
          headStyles: { fillColor: [225, 244, 242], textColor: [8, 75, 70], fontStyle: "bold" },
          columnStyles: { 0: { cellWidth: 40, fontStyle: "bold" }, 1: { cellWidth: contentWidth - 40 } },
          head: [["Purchase Field", "Details"]],
          body: [
            ["Purchase Voucher", purchase.voucher_no || purchase.bill_no || "-"],
            ["Lorry No", lorryNo],
            ["Purchase Date", purchaseDate],
            ["Product", purchase.product_name || purchase.product || purchase.item_name || "-"],
            ["Net Quantity", `${formatDecimal4(qty)} ${purchase.unit || purchase.qty_unit || ""}`.trim()],
            ["Rate", `Rs. ${formatMoney(rate)}`],
            ["Gross Amount", `Rs. ${formatMoney(gross)}`],
          ],
        });
        ...
        autoTable(doc, {
          ...
          margin: { left: margin, right: margin },
          theme: "grid",
          styles: { font: "helvetica", fontSize: 8.2, cellPadding: 2.4, lineColor: [215, 225, 230], textColor: [35, 50, 60] },
          ...
          body: [
            ...deductions.map(([label, value], i) => [i + 1, label, formatMoney(value)]),
            ["", "TOTAL DEDUCTION", formatMoney(deductionTotal)],
          ],
          didParseCell: (data) => {
            if (data.row.index === deductions.length) data.cell.styles.fontStyle = "bold";
          },
        });
        y = (doc.lastAutoTable?.finalY || y) + 5;
        ...
        autoTable(doc, {
          ...
          margin: { left: margin, right: margin },
          theme: "grid",
          styles: { font: "helvetica", fontSize: 9, cellPadding: 3, lineColor: [190, 215, 214] },
          columnStyles: { 1: { halign: "right", fontStyle: "bold" } },
          body: [
            ["Gross Amount", `Rs. ${formatMoney(gross)}`],
            ["Total Deduction", `Rs. ${formatMoney(deductionTotal)}`],
            ["Round Off", `Rs. ${formatMoney(roundOff)}`],
            ["NET PAYABLE", `Rs. ${formatMoney(net)}`],
          ],
          didParseCell: (data) => {
            if (data.row.index === 3) {
              data.cell.styles.fillColor = [8, 96, 88];
        ...
        autoTable(doc, {
          ...
          margin: { left: margin, right: margin },
          theme: "grid",
          styles: { font: "helvetica", fontSize: 9, cellPadding: 3, lineColor: [205, 220, 226] },
          headStyles: { fillColor: [232, 247, 246], textColor: [8, 75, 70], fontStyle: "bold" },
          columnStyles: { 1: { halign: "right", fontStyle: "bold" } },
          head: [["PAYMENT DETAILS", "Amount (Rs.)"]],
          body: [
            ["Payment Date", paymentDate],
            ["Payment Amount", `Rs. ${formatMoney(paid || amount)}`],
            ["Balance After Payment", `Rs. ${formatMoney(balance)}`],
          ],
        });
        y = (doc.lastAutoTable?.finalY || y) + 8;
      });
      ...
      y = ensureSpace(35, y);
      y = sectionTitle("PURCHASE DETAILS", y);
      autoTable(doc, {
        startY: y,
        margin: { left: margin, right: margin },
        theme: "grid",
        styles: { font: "helvetica", fontSize: 8.5, cellPadding: 3, lineColor: [210, 222, 230] },
        body: [["Purchase Bill", row.reference_id || "No purchase bill linked"], ["Payment Date", paymentDate], ["Payment Amount", `Rs. ${formatMoney(amount)}`], ["Balance", `Rs. ${formatMoney(onAccount)}`]],
      });
      y = (doc.lastAutoTable?.finalY || y) + 8;
    }

    y = ensureSpace(48, y);
    doc.setFillColor(236, 250, 248);
    doc.setDrawColor(90, 190, 180);
    doc.roundedRect(margin, y, contentWidth, 30, 4, 4, "FD");
    doc.setTextColor(8, 75, 70);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(9);
    doc.text("FINAL PAYMENT", margin + 6, y + 9);
    doc.setFontSize(16);
    doc.text(`Rs. ${formatMoney(amount)}`, pageWidth - margin - 6, y + 10, { align: "right" });
    doc.setFontSize(8.5);
    doc.setFont("helvetica", "normal");
    doc.text(`Payment Date: ${paymentDate}`, margin + 6, y + 17);
    doc.text(`Balance / On Account: Rs. ${formatMoney(onAccount)}`, margin + 6, y + 24);
    doc.setFont("helvetica", "bold");
    doc.text("Amount in Words:", pageWidth - margin - 80, y + 18, { align: "right" });
    doc.setFont("helvetica", "normal");
    const words = numberToWords(amount);
    const wordLines = doc.splitTextToSize(words, 78);
    doc.text(wordLines, pageWidth - margin - 6, y + 23, { align: "right" });
    y += 38;

    y = ensureSpace(28, y);
    doc.setDrawColor(225, 178, 46);
    doc.setLineWidth(0.8);
    doc.line(margin + 25, y, pageWidth - margin - 25, y);
    doc.setTextColor(8, 96, 88);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(13);
    ...
    doc.text("Thank you for your continued trust and support.", pageWidth / 2, y + 15, { align: "center" });
    doc.text("Computer generated payment voucher", margin, pageHeight - 8);
    doc.text(`Voucher ${row.voucher_no || "-"}`, pageWidth - margin, pageHeight - 8, { align: "right" });
    return doc;
  };
