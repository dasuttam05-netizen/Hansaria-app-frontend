...
export default function WarehouseTradingPage() {
  ...
  const buildLedgerPdf = (ledgerType) => {
    ...
    const partyLabel = isSaleLedger ? "Party" : "Farmer";
    ...
    // Professional cloud-style report header. For Purchase Ledger the account
    // details stay at the top-left and the farmer details are directly below;
    // these fields are deliberately NOT repeated as table columns.
    ...
    doc.text(`Generated: ${formatLedgerDate(new Date().toISOString().slice(0, 10))}`, pageWidth - 14, 18, { align: "right" });

    const safeContact = (details) => [
    ...
    const drawInfoCard = (x, y, w, label, details, accent = [15, 118, 110]) => {
      const lines = safeContact(details);
      const h = 11 + Math.max(1, Math.min(lines.length, 4)) * 4.2 + 4;
      ...
      doc.setTextColor(15, 23, 42);
      doc.setFontSize(10);
      doc.text(String(details.name || "-").slice(0, 95), x + 7, y + 11);
      doc.setFont("helvetica", "normal");
      doc.setFontSize(7.2);
      let cy = y + 15.2;
      lines.slice(0, 4).forEach((line) => {
        const wrapped = doc.splitTextToSize(String(line), w - 12);
        doc.text(wrapped[0], x + 7, cy);
        cy += 4.2;
      });
      return y + h;
    };

    let contentStartY = 25;
    if (isPurchaseLedger) {
      const gap = 5;
      const cardW = (pageWidth - 28 - gap) / 2;
      const accountBottom = drawInfoCard(14, contentStartY, cardW, "Account Details", reportAccount, [15, 118, 110]);
      const farmerBottom = drawInfoCard(14, accountBottom + 4, cardW, "Farmer Details", reportParty, [37, 99, 235]);
      contentStartY = Math.max(farmerBottom, accountBottom) + 7;
    } else if (isSaleLedger) {
      const gap = 5;
      const cardW = (pageWidth - 28 - gap) / 2;
      const accountBottom = drawInfoCard(14, contentStartY, cardW, "Account Details", reportAccount, [15, 118, 110]);
      const partyBottom = drawInfoCard(14 + cardW + gap, contentStartY, cardW, `${partyLabel} Details`, reportParty, [37, 99, 235]);
      contentStartY = Math.max(accountBottom, partyBottom) + 7;
    }

    // Financial summary from the actual ledger rows.
    const entryRows = (displayReportData || []).filter((row) => row.row_type !== "closing");
    ...
    const closingRow = (displayReportData || []).find((row) => row.row_type === "closing");
    const closingAmount = Math.abs(toNumber(closingRow?.balance || 0));
    const summary = [
    ...
    const summaryW = (pageWidth - 28 - 9) / 4;
    summary.forEach(([label, value], i) => {
      const x = 14 + i * (summaryW + 3);
      doc.setFillColor(i === 3 ? 239 : 248, i === 3 ? 246 : 250, i === 3 ? 255 : 252);
      doc.setDrawColor(226, 232, 240);
      doc.roundedRect(x, contentStartY, summaryW, 15, 2, 2, "FD");
      doc.setFont("helvetica", "normal");
      doc.setFontSize(7);
      doc.setTextColor(71, 85, 105);
      doc.text(label, x + 4, contentStartY + 5.5);
      doc.setFont("helvetica", "bold");
      doc.setFontSize(9);
      doc.setTextColor(15, 23, 42);
      doc.text(value, x + 4, contentStartY + 11.5);
    });
    ...
    autoTable(doc, {
      ...
      theme: "grid",
      styles: { fontSize: 6.7, cellPadding: 1.8, overflow: "linebreak", valign: "middle", textColor: [15, 23, 42] },
      headStyles: { fillColor: [15, 118, 110], textColor: [255, 255, 255], fontStyle: "bold", fontSize: 6.7 },
      ...
      body: (displayReportData || []).map((row) => {
        if (isSaleLedger) {
          ...
          return [
            ...
            row.row_type === "closing" ? `Closing (${row.closing_side || ""})` : (row.voucher_type || "-"),
            row.row_type === "closing" ? "" : (row.voucher_no || "-"),
            row.row_type === "closing" ? "" : saleDetails,
            ...
            row.row_type === "closing" ? "" : (row.due_days ?? ""),
            row.row_type === "closing" ? "" : (row.days_overdue ?? ""),
        ...
        return [
          ...
          row.row_type === "closing" ? "" : (row.adjustment_details || "-"),
      ...
      columnStyles: isSaleLedger ? {
        0: { cellWidth: 19 }, 1: { cellWidth: 17 }, 2: { cellWidth: 25 }, 3: { cellWidth: 50 },
        4: { cellWidth: 43 }, 5: { cellWidth: 21 }, 6: { cellWidth: 14 }, 7: { cellWidth: 17 },
        8: { cellWidth: 22 }, 9: { cellWidth: 22 }, 10: { cellWidth: 23 },
      } : {
        0: { cellWidth: 18 }, 1: { cellWidth: 17 }, 2: { cellWidth: 24 }, 3: { cellWidth: 43 },
        4: { cellWidth: 48 }, 5: { cellWidth: 32 }, 6: { cellWidth: 21 }, 7: { cellWidth: 14 },
        8: { cellWidth: 16 }, 9: { cellWidth: 21 }, 10: { cellWidth: 21 }, 11: { cellWidth: 23 },
      },
      didDrawPage: (data) => {
        const pageCount = doc.internal.getNumberOfPages();
        doc.setFontSize(7);
        doc.setTextColor(100, 116, 139);
        doc.text(`Warehouse Trading • ${title}`, 14, doc.internal.pageSize.getHeight() - 7);
        doc.text(`Page ${data.pageNumber} / ${pageCount}`, pageWidth - 14, doc.internal.pageSize.getHeight() - 7, { align: "right" });
      },
    });
    return { doc, title };
  };
