...
const calculateGst = (taxableAmount, gstPercent, supplierState, partyState) => {
  const taxable = Math.max(toNumber(taxableAmount), 0);
  const rate = Math.max(toNumber(gstPercent), 0);
  const supplier = normalizeGstState(supplierState);
  const party = normalizeGstState(partyState);
  const sameState = Boolean(supplier && party && supplier === party);
  const stateReady = Boolean(supplier && party);
  ...
  if (!rate) {
    return {
      ...
      grandTotal: Number(taxable.toFixed(2)),
      sameState,
      stateReady,
      label: "GST NOT SET",
    };
  ...
  if (!stateReady) {
    return {
      gstPercent: rate,
      gstType: "state_not_set",
      cgstPercent: 0,
      sgstPercent: 0,
      igstPercent: 0,
      gstAmount,
      taxableAmount: Number(taxable.toFixed(2)),
      grandTotal: Number((taxable + gstAmount).toFixed(2)),
      sameState: false,
      stateReady: false,
      label: `STATE NOT SET @ ${rate.toFixed(2)}%`,
    };
  }
  ...
  if (sameState) {
    ...
    return {
      ...
      sameState: true,
      stateReady: true,
      label: `CGST ${half.toFixed(2)}% + SGST ${half.toFixed(2)}%`,
    };
  }
...
export default function WarehouseTradingPage() {
  ...
  const [saleTagTargetId, setSaleTagTargetId] = useState("");
  const [saleTagTargetRow, setSaleTagTargetRow] = useState(null);
  ...
  const [selectedSaleReportRow, setSelectedSaleReportRow] = useState(null);
  ...
  const selectedBuyerCompany = companyById.get(String(formData.company_id || formData.buyer_id)) || null;
  ...
  const selectedWarehouseState = selectedWarehouse?.state || "";
  const purchasePartyState = selectedConsignee?.state || selectedFarmerState || "";
  const salePartyState = selectedBuyer?.state || selectedBuyerCompany?.state || selectedConsignee?.state || "";
  ...
  const purchaseGstDetails = calculateGst(
    purchaseGrossAmount,
    selectedProduct?.gst_percent ?? formData.gst_percent,
    selectedWarehouseState,
    purchasePartyState
  );
  const purchaseGrandTotal = purchaseGstDetails.grandTotal;
  const purchaseNetPayable = Math.max(purchaseGrandTotal - purchaseTotalDeduction + purchaseRoundOff, 0);
  const saleGstDetails = calculateGst(
    saleGrossAmountFromData(formData),
    selectedProduct?.gst_percent ?? formData.gst_percent,
    selectedWarehouseState,
    salePartyState
  );
  const saleGrandTotal = saleGstDetails.grandTotal;
  ...
  const purchaseTaggedDeductionDetails = salePurchaseLinks.map((item) => {
    const qty = toNumber(item.quantity || item.weight);
    const fullQty = toNumber(item.purchase_total_qty || item.total_qty || item.purchase_quantity || qty);
    const ratio = fullQty > 0 ? Math.min(qty / fullQty, 1) : 1;
    const getAuto = (key, altKey = null) => toNumber(item[key] ?? (altKey ? item[altKey] : 0)) * ratio;
    const claim = getAuto("claim_amount", "bags_claim");
    const shortage = getAuto("shortage_amount", "shortage_qty");
    const freight = getAuto("transport_charge");
    const others = getAuto("other_deduction", "others");
    const labour = getAuto("labour");
    const moisture = getAuto("moisture");
    const dunki = getAuto("dunki");
    const fungus = getAuto("fungus");
    const discolour = getAuto("discolour");
    const lessBags = getAuto("less_bags_weight");
    const cd = getAuto("cd_amount");
    const adjustment = getAuto("adjustment_amount");
    const tds = getAuto("tds_amount");
    const roundOff = getAuto("round_off");
    const total = toNumber(item.total_deduction ?? item.total_deduct_amount ?? (claim + shortage + freight + others + labour + moisture + dunki + fungus + discolour + lessBags + cd + adjustment + tds));
  ...
  const tagDirectSalePurchase = async (purchase) => {
    const target = saleTagTargetRow || formData || {};
    const tag = buildSalePurchaseTag(purchase, target.farmer_id || target.against_purchase_farmer_id || "");
    ...
    const nextTag = {
      ...
      farmer_id: String(tag.farmer_id || target.farmer_id || target.against_purchase_farmer_id || ""),
      quantity: tag.weight,
      weight: tag.weight,
      rate: tag.rate,
  ...
  const untagDirectSalePurchase = async (purchaseId) => {
    const nextLinks = salePurchaseLinks.filter((item) => String(item.purchase_id) !== String(purchaseId));
    setSalePurchaseLinks(nextLinks);
    if (saleTagTargetId) {
      try {
        await API.put(`/api/wh-vouchers/sale/${saleTagTargetId}`, {
          deduction_only: true,
          against_purchase_enabled: nextLinks.length > 0,
          against_purchase_links: nextLinks,
        });
        toast.success("Purchase bill untagged", { theme: "colored", autoClose: 1800 });
      } catch (err) {
        toast.error(err?.response?.data?.error || "Purchase untag save failed", { theme: "colored", autoClose: 3000 });
      }
    }
  };

  const openSalePurchaseTagging = async (targetSale = null) => {
    const target = targetSale || formData || {};
    const saleId = String(target?.id || target?._id || editId || "").trim();
    setSaleTagTargetId(saleId);
    setSaleTagTargetRow(target);
    if (Array.isArray(target?.against_purchase_links)) {
      setSalePurchaseLinks(target.against_purchase_links);
    } else if (saleId) {
      try {
        const summary = await API.get(`/api/wh-vouchers/sale/${saleId}/summary`);
        setSalePurchaseLinks(Array.isArray(summary.data?.purchase_links) ? summary.data.purchase_links : []);
      } catch {
    ...
    await loadSalePurchaseRows(target);
  ...
  useEffect(() => {
    const handleF10TagPurchaseKey = (event) => {
      ...
      if (activeTab === "reports" && activeReport === "sale" && (salePreviewRow || selectedSaleReportRow)) {
        event.preventDefault();
        void openSalePurchaseTagging(selectedSaleReportRow || salePreviewRow);
      }
    };
  ...
  const loadSalePurchaseRows = async (targetSale = null) => {
    try {
  ...
  const saveVoucher = async () => {
    ...
    try {
      ...
      if (activeVoucherType === "purchase") {
        ...
        payload.gst_percent = purchaseGstDetails.gstPercent;
        payload.gst_type = purchaseGstDetails.gstType;
        payload.cgst_percent = purchaseGstDetails.cgstPercent;
        payload.sgst_percent = purchaseGstDetails.sgstPercent;
        payload.igst_percent = purchaseGstDetails.igstPercent;
        payload.gst_amount = purchaseGstDetails.gstAmount;
        payload.taxable_amount = purchaseGstDetails.taxableAmount;
        payload.grand_total = purchaseGrandTotal;
      ...
      if (activeVoucherType === "sale") {
        ...
        payload.gst_percent = saleGstDetails.gstPercent;
        payload.gst_type = saleGstDetails.gstType;
        payload.cgst_percent = saleGstDetails.cgstPercent;
        payload.sgst_percent = saleGstDetails.sgstPercent;
        payload.igst_percent = saleGstDetails.igstPercent;
        payload.gst_amount = saleGstDetails.gstAmount;
        payload.taxable_amount = saleGstDetails.taxableAmount;
        payload.grand_total = saleGrandTotal;
        ...
        const netAmount = grossAmount + saleGstDetails.gstAmount - claimAmount - otherDeduction - cdAmount - adjustmentAmount - tdsAmount + roundOff;
  ...
  const getPurchaseDeductionDetailsForPdf = (purchase = {}) => {
    const claim = toNumber(purchase.claim_amount || purchase.bags_claim);
    const rows = [
      ["TDS", toNumber(purchase.tds_amount)],
      ["Cash Discount", toNumber(purchase.cd_amount)],
      ["Claim", claim],
    ...
    return rows.filter(([, value]) => value > 0).map(([label, amount]) => ({ label, amount }));
  };
  ...
  const sharePaymentPdfOnWhatsapp = async (item) => {
    try {
      ...
      const message = [
        ...
        `Total Payment: Rs.${formatMoney(row.amount)}`,
        `Adjusted Against Bills: Rs.${formatMoney(adjusted)}`,
        `Balance / On Account: Rs.${formatMoney(balance)}`,
        billLines.length ? "Purchase Bills:" : "",
        ...billLines,
        "Detailed PDF is attached.",
      ].filter(Boolean).join("\n");
  ...
  const showSaleReportPreview = (voucher) => {
    setSelectedSaleReportRow(voucher);
  ...
  const saveSaleVoucherPass = async () => {
    ...
    const payload = {
      ...
      gst_percent: saleGstDetails.gstPercent,
      gst_type: saleGstDetails.gstType,
      cgst_percent: saleGstDetails.cgstPercent,
      sgst_percent: saleGstDetails.sgstPercent,
      igst_percent: saleGstDetails.igstPercent,
      gst_amount: saleGstDetails.gstAmount,
      taxable_amount: saleGstDetails.taxableAmount,
  ...
  const autoFillPaymentAdjustments = (selectedRow = null) => {
    const selectedPurchase = selectedRow?.row || selectedRow || null;

    // When a bill is selected, Auto Adjust applies the payment amount
    // only to that bill. Otherwise keep the existing FIFO-style auto adjust.
    if (selectedPurchase) {
      setPaymentAdjustmentAmount(selectedPurchase, formData.amount);
      return;
    }
  ...
  const selectPaymentBill = (row, key) => {
    const purchaseId = String(row?.id || row?._id || row?.purchase_id || key || "");
    setSelectedPaymentBillId(purchaseId);
    return purchaseId;
  };

  const handlePaymentBillEnter = (row) => {
    if (!row) return;
    selectPaymentBill(row);
    setPaymentAdjustmentAmount(row, formData.amount);
  };
  ...
  const reportColumns = {
    ...
    payment: [
      ...
      ["actions", "Actions", (item) => (
        <div style={{ display: "flex", gap: 7, alignItems: "center", justifyContent: "center", flexWrap: "wrap" }} onClick={(event) => event.stopPropagation()}>
          <button
            type="button"
            onClick={() => handleEditVoucher(item.id || item._id)}
            title="Edit Payment Voucher"
            aria-label="Edit Payment Voucher"
            style={{ width: 34, height: 34, border: "none", borderRadius: 8, background: "#2563eb", color: "#fff", display: "inline-flex", alignItems: "center", justifyContent: "center", cursor: "pointer", boxShadow: "0 2px 6px rgba(0,0,0,.12)", fontSize: 12, fontWeight: 700 }}
          >
            Edit
          </button>
          <button
    ...
    "sale-party-ledger": [
      ...
      ["product", "Product", (item) => (item.row_type === "closing" ? "" : (item.product_name || getProductName(item) || "-"))],
      ...
      ["balance", "Balance", (item) => formatMoney(Math.abs(item.balance || item.bill_balance || 0))],
    ],
    "sale-followup": [
      ["date", "Date", (item) => formatLedgerDate(item.date)],
...
`}</style>
                    ...
                    <div className="purchase-voucher-bottom-panel" style={{ display: "grid", gap: 12 }}>
                      ...
                      <div style={{ border: "1px solid #dbe4ef", borderRadius: 10, background: "#fff", overflow: "hidden" }}>
                        ...
                        <div style={{ padding: 12, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10 }}>
                          <div style={smartInfoBoxStyle}><span>Gross Amount</span><strong>{formatMoney(purchaseGrossAmount)}</strong></div>
                          <div style={smartInfoBoxStyle}><span>GST Amount</span><strong>{formatMoney(purchaseGstDetails.gstAmount)}</strong></div>
                          <div style={smartInfoBoxStyle}><span>Grand Total</span><strong>{formatMoney(purchaseGrandTotal)}</strong></div>
                          <div style={smartInfoBoxStyle}><span>Total Deduction</span><strong>{formatMoney(purchaseTotalDeduction)}</strong></div>
                    ...
                    <div className="purchase-voucher-bottom-panel">
                      <table style={erpMiniTable}>
                        ...
                        <tbody>
                          ...
                          <tr><td style={erpTd}>GST</td><td style={erpTd}>{purchaseGstDetails.label}</td></tr>
                          <tr><td style={erpTd}>GST Amount</td><td style={erpTd}>{formatMoney(purchaseGstDetails.gstAmount)}</td></tr>
                          <tr><td style={{ ...erpTd, fontWeight: 800 }}>Grand Total</td><td style={{ ...erpTd, fontWeight: 800 }}>{formatMoney(purchaseGrandTotal)}</td></tr>
                      ...
                      {formData.sale_type === "direct" && (
                        <>
                          ...
                          <div style={{ ...erpRow, display: "block" }}>
                            <button type="button" onClick={() => { void openSalePurchaseTagging(formData); }} style={{ ...btnAction, background: "#0f766e" }}>
                    ...
                    <div>
                      <table style={erpMiniTable}>
                        ...
                        <tbody>
                          ...
                          /></td></tr>
                          <tr><td style={erpTd}>Other Deduction</td><td style={erpTd}><input name="other_deduction" type="number" step="0.0001" value={String(formData.other_deduction ?? "").trim() === "" ? (saleAutoOtherDeduction ? saleAutoOtherDeduction.toFixed(2) : "0.00") : formData.other_deduction} onChange={handleChange} style={erpCellInput} /></td></tr>
                        ...
                        <div style={{ border: "1px solid #c9d7f0", borderRadius: 8, overflow: "hidden", background: "#fff" }}>
                          ...
                          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
                            <tbody>
                              ...
                              <tr><td style={erpTd}>GST</td><td style={erpTd}>{saleGstDetails.label}</td></tr>
                              <tr><td style={erpTd}>GST Amount</td><td style={erpTd}>{formatMoney(saleGstDetails.gstAmount)}</td></tr>
                              <tr><td style={{ ...erpTd, fontWeight: 800, background: "#f5f8ff" }}>Grand Total</td><td style={{ ...erpTd, fontWeight: 800, background: "#f5f8ff" }}>{formatMoney(saleGrandTotal)}</td></tr>
                            </tbody>
                          </table>
      ...
      {showPaymentAdjustPopup && (
        <div style={modalOverlayStyle}>
          ...
          subtitle="Account → Warehouse → Pending Farmer → Purchase Bills"
            ...
            th={th}
            td={td}
            billSearch={paymentBillSearch}
            onBillSearchChange={setPaymentBillSearch}
            selectedRowKey={selectedPaymentBillId}
            onRowSelect={(row) => selectPaymentBill(row)}
            onEnterAdjust={(row) => handlePaymentBillEnter(row)}
            getRowKey={(row) => row.row?.id || row.row?._id || row.row?.purchase_id || row.key}
            getRowSearchText={(row) => row?.voucher_no || row?.bill_no || ""}
            rows={(partyOutstanding?.purchases || []).filter((row) => toNumber(row.pending_amount) > 0).map((row) => ({
              key: row.id || row._id,
              date: row.date || "-",
              voucher_no: row.voucher_no || row.bill_no || "-",
              bill_no: row.bill_no || row.voucher_no || "-",
              warehouse: getWarehouseName(row),
      ...
      {showSalePurchaseTagModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 1200, background: "rgba(15, 23, 42, 0.5)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ width: "min(1080px, 100%)", maxHeight: "88vh", overflow: "auto", background: "#fff", borderRadius: 10, padding: 18, boxShadow: "0 20px 50px rgba(15, 23, 42, 0.28)" }}>
                ...
                <tbody>
                  {filteredSalePurchaseTagRows.map((purchase) => {
                    ...
                    return <tr key={purchase.purchase_id || purchase.id || purchase._id}>
                      ...
                      <td style={td}>{showTaggedSalePurchases ? <button type="button" onClick={() => void untagDirectSalePurchase(purchase.purchase_id || purchase.id || purchase._id)} style={{ ...btnAction, background: "#dc2626", padding: "4px 8px", fontSize: 11 }}>Untag</button> : <button type="button" onClick={() => void tagDirectSalePurchase(purchase)} style={{ ...btnAction, background: "#1d4ed8", padding: "4px 8px", fontSize: 11 }}>Tag</button>}</td>
                    </tr>;
                  })}
