// src/pages/InwardManagementPage.jsx
...
export default function InwardManagementPage() {
  const API = "/api";
  ...
  const fetchInwards = async () => {
    try {
      const res = await axios.get(`${API}/inward`);
      setInwards(res.data);
    } catch (err) {
      console.error(err);
      alert("Error fetching inwards");
    }
  };
  ...
  return (
    <div style={{ fontFamily: "Segoe UI, Arial, sans-serif", padding: "16px" }}>
      <h2 style={{ marginTop: 0, marginBottom: "12px" }}>📦 Inward Management</h2>
        ...
        >
          ...
          >
            ...
            <h3 style={{ marginTop: "8px", marginBottom: "20px", fontSize: "22px" }}>
              {editData ? "✏️ Edit Inward Entry" : "➕ New Inward Entry"}
            </h3>
            ...
            >
              ...
              <select name="product_id" value={formData.product_id} onChange={handleChange}
                ...
                {products.map(p => <option key={getRecordId(p)} value={getRecordId(p)}>{p.name}</option>)}
              </select>

              <select name="company_id" value={formData.company_id} onChange={handleChange}
                style={{ padding:"12px", fontSize:"16px", borderRadius:"6px", border:"1px solid #ccc" }}>
                <option value="">Select Company</option>
                {companies.map(c => <option key={getRecordId(c)} value={getRecordId(c)}>{c.name}</option>)}
              </select>
              ...
              <div style={{ gridColumn:"1/-1", display:"flex", justifyContent:"flex-end", gap:"12px", marginTop:"12px" }}>
                <button type="submit" style={{ background:"#4caf50", color:"#fff", padding:"10px 16px", border:"none", borderRadius:"6px", cursor:"pointer", fontSize:"16px" }}>Save</button>
